...
/*  population of the random vectors should occur in here, consider changing flags 
  to map<string, bool> to include long_opt flags. Since this is going to be the 
  final called function in main, there is no need to worry about how a vector
  of flats, as is created in the case of bucket sort, needs to escape the function's
  scope.

  1) Make if block for flags to determine which sorts are set.
  2) Make random vectors of ints if unsorted is empty. This should occur before (1)
  3) Make random vector of floats if unsorted is empty and bucket flag is set
  4) If "size" flag is set, no need to do (3) or (4)
  5) Start making code for sort class objects, starting with insertion sort.
*/  
}
